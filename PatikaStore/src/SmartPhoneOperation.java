...
public class SmartPhoneOperation extends SmartPhone {

    private static TreeSet<SmartPhone> modelSet = new TreeSet<>(new SmartPhoneIDComp());
    private static TreeSet<SmartPhone> modelSetByBrands = new TreeSet<>(new SmartPhoneBrandComp());

    public SmartPhoneOperation(int id, double price, int discountRate, int stock, String name, int memory, double screenSize, int ram, Brand brand, int battery, String color) {
        super(id, price, discountRate, stock, name, memory, screenSize, ram, brand, battery, color);
    }

    public static void modelsOfMobilePhones() {
        SmartPhone galaxya51 = new SmartPhone(1, 3199.0, 20, 15, "Galaxy A51", 128, 6.5, 6, Brand.samsung, 4000, "Siyah");
        modelSet.add(galaxya51);
        SmartPhone iphone11 = new SmartPhone(2, 7379.0, 15, 25, "iPhone 11", 64, 6.1, 6, Brand.apple, 3046, "Mavi");
        modelSet.add(iphone11);
        SmartPhone redmiNote10 = new SmartPhone(3, 4012.0, 30, 55, "Redmi Note 10 Pro", 128, 6.5, 12, Brand.xiaomi, 4000, "Beyaz");
    ...
    public static void printMPModelsSortByBrands() {
        ...
        if (modelSetByBrands.isEmpty()) {
            ...
            for (SmartPhone print : modelSetByBrands) {
                System.out.println("\uD83C\uDD94 Telefon ID: " + print.getId() + "\n\uD83D\uDCF1 Markası: " + print.getBrand().getName() + "\n\uD83D\uDDB4 Depolama Alanı: " + print.getMemory() + " GB" + "\n\uD83D\uDDB5 Ekran Boyutu: " + print.getScreenSize() + " inch" + "\n\uD83E\uDEAB Pil Kapasitesi: " + print.getBattery() + " MHZ" + "\n\uD83D\uDCDF RAM: " + print.getRam() + " GB" + "\n\uD83C\uDF08 Renk Seçeneği: " + print.getColor() + "\n\uD83D\uDCE6 Stok durumu: " + print.getStock() + "\n\uD83D\uDCB5 Fiyatı: " + print.getTotal() + "₺");
                System.out.println("###########################");
                System.out.println();
            }
    ...
    public static void deleteModelById(int id) {
        ...
        SmartPhone modelToDelete = null;
        for (SmartPhone model : modelSet) {
            if (model.getId() == id) {
                modelToDelete = model;
                break;
            }
        ...
        // Silinecek olan nesne setin niçerisinde kontrol ediliyor
        for (SmartPhone model2 : modelSetByBrands) {
            if (model2.getId() == id) {
    ...
    public static void addItem() {
        ...
        try {
            ...
            SmartPhone newPhone = new SmartPhone(id, price, discountRate, stock, name, memory, screenSize, ram, selectedBrand, battery, color);
